...
export default async function DoctorSupportPage() {
  ...
  const { data } = await supabase.from("dv_support_tickets").select("*").eq("user_id", user.id).order("created_at", { ascending: false });
  const tickets: SupportTicket[] = (data || []) as SupportTicket[];

  return (
    <div className="space-y-8">
      <SectionHeader title="Support" subtitle="Contact the platform team for any issues" />
