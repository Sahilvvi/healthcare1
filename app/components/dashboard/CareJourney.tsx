...
function getTimelineDate(timelines: CaseTimeline[], stage: string) {
  ...
  return t?.created_at ? new Date(t.created_at).toLocaleDateString("en-IN", { day: "numeric", month: "short" }) : null;
}
    ...
    <div className="rounded-2xl border border-border bg-white p-6 shadow-sm">
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted">{activeCase?.category} · Last updated {getTimelineDate(timelines, activeCase?.status || "NEW") || "today"}</p>
        </div>
        <span
      ...
      <div className="h-2 overflow-hidden rounded-full bg-sage">
        <div
          className="h-full rounded-full bg-teal transition-all duration-1000"
          style={{ width: `${progress}%` }}
        />
      </div>

      <div className="relative mt-8">
        <div className="absolute left-4 top-0 h-full w-0.5 bg-sage md:left-5" />
        {journey.map((step, i) => {
          const date = getTimelineDate(timelines, step.key);
          return (
            <div key={step.key} className="relative mb-8 flex gap-4 last:mb-0">
              <div
                className={`z-10 flex h-10 w-10 shrink-0 items-center justify-center rounded-full border-2 transition-colors md:h-12 md:w-12 ${
              ...
              <div className="flex-1 pt-1 md:pt-2">
                <p
                  className={`font-heading text-sm font-semibold md:text-base ${
                ...
                {i === currentIndex && (
                  <p className="mt-2 text-xs text-teal">Current stage</p>
                )}
              </div>
            </div>
          );
