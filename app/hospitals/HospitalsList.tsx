...
export default function HospitalsList({ hospitals }: { hospitals: Hospital[] }) {
  ...
  const allCities = useMemo(
    () =>
      Array.from(
        new Set(hospitals.map((h) => h.city).filter((c): c is string => Boolean(c)))
      ).sort(),
    [hospitals]
  );
