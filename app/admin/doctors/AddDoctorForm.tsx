...
export function AddDoctorForm() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const result = await addDoctor(new FormData(e.currentTarget));
    setLoading(false);
    setStatus(result);
  ...
  return (
    <form onSubmit={handleSubmit} className="mb-8 rounded-lg border border-border bg-white p-6 shadow-sm">
      ...
      {status?.ok && <p className="mt-3 text-sm text-teal">Doctor added.</p>}
      <div className="mt-4 grid gap-4 sm:grid-cols-2">
        <input name="name" placeholder="Full name" required className="rounded-md border border-border px-4 py-2 text-sm outline-none focus:border-teal" />
        ...
        <input name="experience" placeholder="Experience (e.g. 15+ years)" className="rounded-md border border-border px-4 py-2 text-sm outline-none focus:border-teal" />
        <input name="procedures" placeholder="Procedures (e.g. 1,200+)" className="rounded-md border border-border px-4 py-2 text-sm outline-none focus:border-teal" />
        <input name="availability" placeholder="Availability" className="rounded-md border border-border px-4 py-2 text-sm outline-none focus:border-teal" />
